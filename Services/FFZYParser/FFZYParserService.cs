...
namespace VodDownloader.Services.FFZYParser;
...
public sealed class FFZYParserService : ISiteParserService {
    ...
    public VodDetailLite? GetVodDetail (UInt32 vid) {
        ...
        if (String.IsNullOrWhiteSpace(httpBody)) {
            Console.WriteLine("[FFZYParserService] 获取数据失败，响应为空");
            return null;
        }
        ...
        try {
            ...
            Console.WriteLine("[FFZYParserService] 解析数据失败");
            Console.WriteLine(ex.Message);
            return null;
        }
        ...
        if (response.List.Count < 1) {
            Console.WriteLine("[FFZYParserService] 剧集为空");
            return null;
        }
